...
class SNMF():
    ...
    def sgd_solver(self, max_iter = 100):
        if(self.batch_number == 1):        # normal MUR
            ...
            for iter in range(max_iter):  # stochastic MUR         
...
A_nmf = SNMF(A, r=cluster_num,  h_init = initial_h, batch_number=3) # call snmf's constructor
...
print("Start running...")
t0 = time()
result = A_nmf.sgd_solver(max_iter=2000)                              # run gd, return h
t1 = time()
